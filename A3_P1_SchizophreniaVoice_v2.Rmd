...
dfo = lapply(files, read.delim)
dfi = lapply (dfo, summary)
dinal_df = data.frame()

for (i in x) {
  f = read.delim(files[i])
  mean = mean(i)
  standard_dev = sd(i) 
  range = range(i) 
  median = median(i)
  ...
  recur_plot = r_plot(i)
  final_df = rbind(final_df, data.frame(f, mean, standard_dev, range, median, iqr, aad, coef_var, recur_plot))
